...
import cn.dlb.cm.tool.ExcelReader;
...
public class ContractRecordController extends BaseController {
	...
	public String advancedSearch(Query_ContractRecord contractRecord,
		...
		System.out.println(signingDateStart+"--------------------"+contractRecord.toString());
	...
	public String outputData(String[] id,HttpServletResponse response){
		...
		try {
			...
			try {
				ExcelReader.exportExcel(outputStream,outputDataList);
			} catch (Exception e) {
				e.printStackTrace();
			} 
		} catch (IOException e) {
	...
	//导入数据
		...
		public String inputData(@RequestParam("file")MultipartFile file){
			try {
				...
				List <ContractRecord> list=ExcelReader.importExcel(file.getInputStream());
